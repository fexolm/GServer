...
namespace GServer.Plugins
{
        ...
        where TModel : AccountModel, new()
    {
        ...
        private void AccountLoginHandler(Connection.Connection connection, Guid accountId) {
            lock (_accounts) {
        ...
        private void InfoRequestHandler(Message msg, Connection.Connection con) {
            lock (_accounts) {
                if (_accounts.ContainsKey(con.Token)) {
                    _host.Send(
                        new Message((short) AccountMessage.InfoResponse, Mode.None, _accounts[con.Token].Serialize()),
                        con);
                }
            }
        }

...
namespace GServer.Plugins.Lobby
{
        ...
        where TGame : Game<TAccountModel>, new()
    {
        ...
        protected override void InitializeHandlers() {
            ...
            _host.AddHandler((short) LobbyMessages.GetRooms, (m, c) => {
                lock (_rooms) {
                    _host.Send(
                        new Message((short) LobbyMessages.GetRoomsResponse, Mode.Reliable,
                            DataStorage.CreateForWrite().Push(_rooms.Keys.ToList().Serialize())), c);
                }
            });

            AddHandler((short) LobbyMessages.StartGame, (msg, room, account) => { room.StartGame(); });
        }
        ...
        private void PlayerLeavedHander(LobbyRoom<TAccountModel, TGame> room, TAccountModel player) {
            foreach (var p in room.Players) {
                if (p != player) {
                    _host.Send(
                        new Message((short) LobbyMessages.PlayerLeaved, Mode.Reliable,
        ...
        public override void AddHandler(short messageType,
            Action<Message, LobbyRoom<TAccountModel, TGame>, TAccountModel> roomHandler) {
        ...
        where TAccountModel : AccountModel, new()
    {
        ...
        public void JoinRoom(Token roomToken) {
            _host.Send(new Message((short) LobbyMessages.JoinRoom, Mode.Reliable,
                DataStorage.CreateForWrite().Push(roomToken)));
        }

...
{
    public class
        MatchmakingManager<TGame, TAccountModel> : RoomManager<TGame, TAccountModel,
            MatchmakingRoom<TAccountModel, TGame>>
        ...
        where TGame : Game<TAccountModel>, new()
    {
        ...
        private void ManageRoom(TAccountModel[] clients) {
            ...
            foreach (var player in room.Players) {
                _host.Send(new Message((short) MatchmakingMessages.GameFound, Mode.Reliable, room.RoomToken),
                    player.Connection);
            }
            room.RoomClosed += () => {
                lock (_rooms) {
